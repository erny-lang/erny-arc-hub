import { useState, useRef, useEffect } from "react";
import { useWallet } from "../context/WalletContext";

function truncAddr(address: string) {
  return `${address.slice(0, 6)}...${address.slice(-4)}`;
}

export default function Header() {
  const { address, isConnecting, connect, disconnect, changeWallet } = useWallet();
  const [open, setOpen] = useState(false);
  const ref = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const close = (e: MouseEvent) => {
      if (ref.current && !ref.current.contains(e.target as Node)) setOpen(false);
    };
    document.addEventListener("mousedown", close);
    return () => document.removeEventListener("mousedown", close);
  }, []);

  return (
    <header
      className="flex items-center justify-between px-6 h-14 border-b"
      style={{ background: "#0d3f99", borderColor: "rgba(161, 198, 255, 0.35)" }}
    >
      <div className="flex items-center gap-2.5">
        <div className="h-7 flex items-center">
          <svg
            width="146"
            height="50"
            viewBox="0 0 146 50"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="h-7 w-auto"
            aria-label="Arc"
          >
            <path
              d="M23.8574 0C31.0115 0 37.371 6.19775 41.7656 17.4521C44.0513 23.3056 45.7332 30.2603 46.7295 37.8262C46.8186 38.5019 46.8939 39.1888 46.9717 39.874C46.9969 39.9162 47.0119 39.9553 47.0068 39.9873C47.0068 39.9873 47.5924 43.6447 47.7168 50H47.6514C46.7829 49.2873 36.54 41.2389 19.5615 43.5693C19.8177 40.6962 20.1699 37.9004 20.625 35.2207C20.6482 35.0838 20.6755 34.9514 20.6992 34.8154C27.3585 34.6146 33.1876 35.3879 37.6572 36.4014C37.6406 36.2954 37.6263 36.1865 37.6094 36.0811C36.6906 30.3599 35.3355 25.1217 33.5879 20.6455C30.7304 13.3264 27.001 8.77832 23.8574 8.77832C20.7141 8.77863 16.9853 13.3266 14.1279 20.6455C13.4363 22.4157 12.8068 24.3036 12.2422 26.2949C11.4483 29.0854 10.7807 32.0773 10.248 35.2207C9.45968 39.8629 8.96755 44.8418 8.78613 50H0C0.405408 37.7593 2.48104 26.3352 5.9502 17.4521C10.3437 6.19798 16.7036 0.000184295 23.8574 0ZM131.198 16.6523C134.06 16.6524 136.482 17.1955 138.466 18.2783C140.448 19.3622 142.002 20.7725 143.127 22.5098C144.251 24.2481 144.957 26.0777 145.243 27.999L141.624 28.7354C141.419 27.1408 140.908 25.6789 140.091 24.3496C139.272 23.0217 138.128 21.9584 136.656 21.1611C135.184 20.3638 133.365 19.9649 131.198 19.9648C129.031 19.9648 127.088 20.4664 125.371 21.4678C123.654 22.4702 122.294 23.8806 121.293 25.6992C120.291 27.5192 119.79 29.6764 119.79 32.1699V32.6602C119.79 35.1546 120.291 37.312 121.293 39.1309C122.295 40.9507 123.654 42.3619 125.371 43.3633C127.088 44.3657 129.031 44.8662 131.198 44.8662C134.469 44.8662 136.963 44.0174 138.681 42.3203C140.398 40.624 141.461 38.5481 141.87 36.0947L145.488 36.8311C145.12 38.7534 144.354 40.5828 143.188 42.3203C142.023 44.0587 140.448 45.4699 138.466 46.5527C136.482 47.6355 134.06 48.1777 131.198 48.1777C128.295 48.1777 125.709 47.5534 123.439 46.3066C121.17 45.0599 119.381 43.2709 118.073 40.9404C116.764 38.6097 116.11 35.8707 116.11 32.7217V32.1084C116.11 28.9191 116.764 26.1698 118.073 23.8594C119.381 21.5499 121.17 19.7711 123.439 18.5234C125.709 17.2767 128.295 16.6523 131.198 16.6523ZM97.2402 47.3193H93.1309L89.4512 35.9111H69.9473L66.2666 47.3193H62.1582L76.2031 4.38672H83.1943L97.2402 47.3193ZM116.065 20.8232H112.14C109.89 20.8233 108.091 21.4578 106.742 22.7246C105.393 23.9925 104.719 25.9754 104.719 28.6738V47.3193H101.038V17.5117H104.596V21.2529H105.332C105.904 19.9035 106.752 18.9219 107.877 18.3086C109.001 17.6953 110.566 17.3887 112.569 17.3887H116.065V20.8232ZM71.0508 32.3545H88.3467L80.0664 6.7168H79.3311L71.0508 32.3545Z"
              fill="url(#paint0_linear_480_17187)"
            />
            <defs>
              <linearGradient id="paint0_linear_480_17187" x1="72.7441" y1="-3.33531e-06" x2="88.5223" y2="100.812" gradientUnits="userSpaceOnUse">
                <stop stopColor="white" />
                <stop offset="1" stopColor="white" stopOpacity="0" />
              </linearGradient>
            </defs>
          </svg>
        </div>
        <div className="leading-none">
          <div className="text-[12px] font-semibold tracking-wide text-white">erny</div>
          <div className="text-[10px] font-medium text-white mt-[2px]">hub</div>
        </div>
      </div>

      <div className="flex items-center gap-2">
        {address ? (
          <>
            <div className="flex items-center gap-1.5 h-[30px] pl-1 pr-3 rounded-full bg-surface-2 border border-border">
              <div className="w-5 h-5 rounded-full bg-gradient-to-br from-purple to-pink-500" />
              <span className="text-[11px] font-mono text-text-2">{truncAddr(address)}</span>
            </div>
            <div className="relative" ref={ref}>
              <button
                onClick={() => setOpen(!open)}
                className="w-[30px] h-[30px] rounded-full bg-surface-2 border border-border flex items-center justify-center hover:border-border-light transition-colors cursor-pointer"
              >
                <svg className="w-3 h-3 text-text-3" viewBox="0 0 16 16" fill="currentColor">
                  <circle cx="3" cy="8" r="1.3" />
                  <circle cx="8" cy="8" r="1.3" />
                  <circle cx="13" cy="8" r="1.3" />
                </svg>
              </button>
              {open && (
                <div className="absolute right-0 top-full mt-1.5 w-52 card p-1 z-50 animate-fade-in shadow-2xl shadow-black/60">
                  <button
                    onClick={() => {
                      navigator.clipboard.writeText(address);
                      setOpen(false);
                    }}
                    className="w-full flex items-center gap-2 px-3 py-2 rounded-[8px] text-[12px] text-text-2 hover:bg-surface-2 transition-colors cursor-pointer"
                  >
                    Copy Address
                  </button>
                  <a
                    href={`https://testnet.arcscan.app/address/${address}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    onClick={() => setOpen(false)}
                    className="w-full flex items-center gap-2 px-3 py-2 rounded-[8px] text-[12px] text-text-2 hover:bg-surface-2 transition-colors cursor-pointer"
                  >
                    View on Explorer
                  </a>
                  <button
                    onClick={() => {
                      setOpen(false);
                      changeWallet();
                    }}
                    className="w-full flex items-center gap-2 px-3 py-2 rounded-[8px] text-[12px] text-text-2 hover:bg-surface-2 transition-colors cursor-pointer"
                  >
                    Change Wallet
                  </button>
                  <div className="mx-2 my-0.5 border-t border-border" />
                  <button
                    onClick={() => {
                      disconnect();
                      setOpen(false);
                    }}
                    className="w-full flex items-center gap-2 px-3 py-2 rounded-[8px] text-[12px] text-red hover:bg-red-dim transition-colors cursor-pointer"
                  >
                    Disconnect
                  </button>
                </div>
              )}
            </div>
          </>
        ) : (
          <button
            onClick={connect}
            disabled={isConnecting}
            className="h-[30px] px-4 rounded-lg text-[11px] font-semibold transition-colors cursor-pointer disabled:opacity-40"
            style={{ background: "#fff", color: "#000", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 8 }}
          >
            {isConnecting ? "Connecting..." : "Connect Wallet"}
          </button>
        )}
      </div>
    </header>
  );
}
